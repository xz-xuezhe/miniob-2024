...
RC BplusTreeIndex::create(Table *table, const char *file_name, const IndexMeta &index_meta, const vector<const FieldMeta *> &field_metas)
{
  ...
  RC rc = index_handler_.create(table->db()->log_handler(),
      bpm,
      file_name,
      field_metas.size() == 1 ? field_metas[0]->type() : AttrType::UNDEFINED,
      field_metas_len_);
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to create index_handler, file_name:%s, index:%s, rc:%s", file_name, index_meta.name(), strrc(rc));
    return rc;
  }
...
RC BplusTreeIndex::open(Table *table, const char *file_name, const IndexMeta &index_meta, const vector<const FieldMeta *> &field_metas)
{
  ...
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to open index_handler, file_name:%s, index:%s, rc:%s", file_name, index_meta.name(), strrc(rc));
    return rc;
  }
...
RC BplusTreeIndex::destroy()
{
  RC rc = RC::SUCCESS;
  if (inited_) {
    LOG_INFO("Begin to destroy index, index:%s", index_meta_.name());
...
{
  char *data = new char[field_metas_len_];
  int offset = 0;
  for (const FieldMeta &field_meta : field_metas_) {
    int len = field_meta.len();
    if (field_meta.type() == AttrType::CHARS) {
      len = 0;
      while (len < field_meta.len() && record[len])
        ++len;
    }
    memcpy(data + offset, record + field_meta.offset(), len);
